...
class UserAuthApp:
    ...
    def create_main_widgets(self):
        ...
        login_button = tk.Button(self.root, text="Log In Now",
                                  ...
                                  relief=tk.FLAT,
                                  padx=20, pady=10)
        self.canvas.create_window(200, 400, window=login_button)
        ...
        if os.path.exists("tasks.json"):  # Periksa apakah file tasks.json ada
            with open("tasks.json", "r") as f:
                ...
                if data:  # Jika file tidak kosong
                    try:
                        all_tasks = json.loads(data)
                        # Filter tugas berdasarkan username
                        self.daftar_tugas = [task for task in all_tasks if task['username'] == self.username]
