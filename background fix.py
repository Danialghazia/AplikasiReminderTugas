...
class AplikasiPengingatTugas:
    def __init__(self, akar, username):
        ...
        self.akar.attributes('-fullscreen', True)
        self.akar.geometry("800x600")
        
        # Load background image
        self.bg_image = Image.open("14.png")  # Ganti dengan path gambar Anda
        self.bg_image = self.bg_image.resize((800, 600), Image.Resampling.LANCZOS)
        self.bg_image_tk = ImageTk.PhotoImage(self.bg_image)

        # Create a canvas to display the background image
        self.canvas = tk.Canvas(self.akar, width=1920, height=1080)
        self.canvas.pack(fill="both", expand=True)

        # Add the background image to the canvas
        self.canvas.create_image(0, 0, image=self.bg_image_tk, anchor="nw")

        # Now create the main frame for the task input
        self.create_main_widgets()
        # Font settings
